...
class MonitorAlert(db.Model):
    ...
    # Source: where this alert came from
    # "monitor"      — generated by a Monitor's scheduled comparison
    # "finding"      — manually escalated from a Finding (no monitor)
    # "lookup_tool"  — saved from a public LookUp Tool result
    source = db.Column(db.String(20), nullable=False, default="monitor", index=True)
    source_tool = db.Column(db.String(50), nullable=True)         # e.g. "cert-lookup"
    source_target = db.Column(db.String(500), nullable=True)      # the input value (URL, IP, etc.)

    # Alert classification
    alert_type = db.Column(db.String(30), nullable=False)  # new_finding, severity_change, resolved, manual
