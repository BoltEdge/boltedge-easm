      ...
      <div className="p-8">
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-card border border-border rounded-xl overflow-hidden">
              ...
              {settings.emailEnabled && (
                <div className="border-t border-border p-5 space-y-3 bg-muted/10">
                  ...
                  {/* Send test alert — verifies email delivery without
                      waiting for a real monitor to fire. The current settings
                      may be unsaved; the user gets a clear hint about that. */}
                  {canEdit && settings.emailRecipients.length > 0 && (
                    <div className="pt-2 border-t border-border/60 flex items-center justify-between gap-3">
                      <span className="text-[11px] text-muted-foreground">
                        Verify delivery by sending a sample alert to all recipients.
                      </span>
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => setBanner({ kind: "ok", text: `Test alert queued to ${settings.emailRecipients.length} recipient${settings.emailRecipients.length === 1 ? "" : "s"}. Check your inbox in a minute.` })}
            ...
            <div className="bg-card border border-border rounded-xl overflow-hidden">
              ...
              {settings.webhookEnabled ? (
                <div className="border-t border-border p-5 bg-muted/10 space-y-3">
                  <div>
                  ...
                  {canEdit && settings.webhookUrl.trim().length > 0 && (
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-[11px] text-muted-foreground">
                        Verify the URL accepts our payload by POSTing a sample alert.
                      </span>
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => setBanner({ kind: "ok", text: "Test alert POSTed. Check the destination — Slack/Discord/PagerDuty etc. — for the sample payload." })}
                        className="border-border text-foreground hover:bg-accent shrink-0"
                      >
                        <Webhook className="w-3.5 h-3.5 mr-1.5" />Send test
                      </Button>
                    </div>
                  )}
                </div>
              ) : (
                /* Off-state hint — clarifies what the toggle on the right
                   does and shows where the URL field will appear. */
                <div className="border-t border-border px-5 py-3 bg-muted/10 text-[11px] text-muted-foreground flex items-center gap-2">
                  <Info className="w-3 h-3 flex-shrink-0" />
                  Flip the toggle to set a destination URL. Compatible with Slack incoming webhooks, Discord, PagerDuty Events API, or any HTTP endpoint that accepts JSON.
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 mb-1">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {/* All-selected state confirmation — easy to miss otherwise. */}
                    {settings.notifyOnSeverity.length === SEVERITY_ORDER.length && (
                      <span className="text-[10px] font-semibold text-[#10b981] bg-[#10b981]/10 border border-[#10b981]/30 px-1.5 py-0.5 rounded">
                        All selected
                      </span>
                    )}
                    {settings.notifyOnSeverity.length === 0 && (
                      <span className="text-[10px] font-semibold text-amber-300 bg-amber-500/10 border border-amber-500/30 px-1.5 py-0.5 rounded">
                        None selected — no alerts will fire
                      </span>
                    )}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    Only notify for selected severity levels
                    <span className="text-muted-foreground/60"> · {settings.notifyOnSeverity.length}/{SEVERITY_ORDER.length} levels active</span>
                  </div>
                </div>
                {canEdit && (
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button
                      type="button"
                      onClick={() => setSettings({ ...settings, notifyOnSeverity: [...SEVERITY_ORDER] as any })}
                      disabled={settings.notifyOnSeverity.length === SEVERITY_ORDER.length}
                      className="text-[11px] text-muted-foreground hover:text-foreground disabled:opacity-30 disabled:cursor-not-allowed underline-offset-2 hover:underline"
                    >
                      Select all
                    </button>
                    <span className="text-muted-foreground/40">·</span>
                    <button
                      type="button"
                      onClick={() => setSettings({ ...settings, notifyOnSeverity: [] })}
                      disabled={settings.notifyOnSeverity.length === 0}
                      className="text-[11px] text-muted-foreground hover:text-foreground disabled:opacity-30 disabled:cursor-not-allowed underline-offset-2 hover:underline"
                    >
                      Clear
                    </button>
            ...
            {/* Plan info — clarified phrasing. The previous "Every 7 days
                scans — Starter" read awkwardly; this version reads as a
                proper sentence and makes clear that the cadence is a
                plan-level cap (not a setting). */}
            <div className="bg-card border border-border rounded-xl p-5">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground">
                    Monitors rescan {monitoringFrequencyLabel(monitoringFrequency).toLowerCase()} on the {planLabel} plan.
                  </div>
                </div>
                <span className="ml-auto px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase bg-primary/10 text-primary flex-shrink-0">
                  {planLabel}
                </span>
              </div>
            </div>
